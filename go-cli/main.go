// Command Nominatim-cli is a CLI + REPL for the Nominatim SDK,
...
const prompt = "nominatim"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "address_lookup administrative debug reverse search server_status"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NOMINATIM_APIKEY (api key), NOMINATIM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NOMINATIM_APIKEY carries the API key and
	// NOMINATIM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NOMINATIM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NOMINATIM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNominatimSDK(opts)
