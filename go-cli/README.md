...
boru-driven command-line client **and** interactive REPL for the Nominatim
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nominatim-cli)
make build
...
./nominatim-cli list administrative

# 5. Override the API base URL for a single call
NOMINATIM_BASE=https://api.example.com ./nominatim-cli list address_lookup

# 6. No arguments -> interactive REPL
./nominatim-cli
nominatim> list address_lookup
nominatim> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nominatim-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nominatim-cli list address_lookup
   ```
...
Run with no arguments to open a REPL (prompt `nominatim>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `address_lookup`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nominatim-cli`. |
...
(`./nominatim-cli <expr>`) and interactively (the REPL), and expressions compose the
