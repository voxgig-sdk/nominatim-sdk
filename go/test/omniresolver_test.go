...
import (
	...
	omni "github.com/voxgig-sdk/nominatim-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NominatimError); is {
				out := map[string]any{
					"name":    "NominatimError",
...
func omniCtx(arg any, client *sdk.NominatimSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NominatimSDK); is {
			client = live
