...
func (sdk *NominatimSDK) OptionsMap() map[string]any {
...
func (sdk *NominatimSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *NominatimSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *NominatimSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *NominatimSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *NominatimSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *NominatimSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("NominatimSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *NominatimSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *NominatimSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("NominatimSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
