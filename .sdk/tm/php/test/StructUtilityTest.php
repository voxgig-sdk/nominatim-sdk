...
// Nominatim SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = NominatimOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(NominatimOmni::structfix($now, Struct::undef()))
                        !== json_encode(NominatimOmni::structfix($val, Struct::undef()))) {
