...
{
    private static function utility(): NominatimUtility
    {
        return new NominatimUtility();
    }
    ...
    private static function ctx(array $over = []): NominatimContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new NominatimContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new NominatimOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof NominatimError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new NominatimOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = NominatimMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new NominatimOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = NominatimMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new NominatimOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = NominatimMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NominatimSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = NominatimMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new NominatimResponse([]),
            'result' => new NominatimResult([]),
        ]);
        [, $err] = NominatimMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new NominatimSpec([]),
            'response' => null,
            'result' => new NominatimResult([]),
        ]);
        [, $err] = NominatimMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new NominatimSpec(['step' => 's']),
            'response' => new NominatimResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new NominatimResult([]),
        ]);
        [, $err] = NominatimMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new NominatimSpec(['step' => 's']),
            'response' => new NominatimResponse(self::resp(200, ['v' => 1])),
            'result' => new NominatimResult([]),
        ]);
        [, $err] = NominatimMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new NominatimSpec(['step' => 's']),
            'response' => new NominatimResponse(self::resp(200, ['v' => 2])),
            'result' => new NominatimResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        NominatimMakeResponse::call($ctx);
    ...
    {
        $preset = new NominatimResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new NominatimSpec([]),
            'response' => new NominatimResponse([]),
            'result' => new NominatimResult([]),
        ...
        [$got, $err] = NominatimMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new NominatimResult([])]);
        [, $err] = NominatimMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new NominatimSpec([]), 'result' => null]);
        [, $err] = NominatimMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NominatimSpec(['step' => 's']),
            'result' => new NominatimResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new NominatimOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NominatimMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NominatimSpec(['step' => 's']),
            'result' => new NominatimResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new NominatimOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NominatimMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NominatimResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new NominatimSpec([]),
            'result' => new NominatimResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = NominatimMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NominatimMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new NominatimError('boom', 'boom');
        $utility->fetcher = function (NominatimContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NominatimSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = NominatimMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (NominatimContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = NominatimMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(NominatimResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (NominatimContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NominatimSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = NominatimMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new NominatimResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new NominatimSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = NominatimMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = NominatimMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = NominatimMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new NominatimResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, NominatimDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new NominatimResult(['ok' => false])]);
        $this->expectException(NominatimError::class);
        NominatimDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new NominatimResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', NominatimMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        NominatimMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new NominatimBaseFeature();
        $b = new NominatimBaseFeature();
        NominatimFeatureAdd::call($ctx, $a);
        NominatimFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): NominatimBaseFeature
    {
        $f = new NominatimBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        NominatimFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): NominatimContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new NominatimSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NominatimPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new NominatimResponse(['headers' => null]),
            'result' => new NominatimResult([]),
        ]);
        NominatimResultHeaders::call($ctx);
