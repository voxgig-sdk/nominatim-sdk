...
class NominatimNetsimFeature extends NominatimBaseFeature
{
    ...
    private function _simulate(NominatimContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(NominatimContext $ctx, array $applied): void
    {
        $client = $this->client;
