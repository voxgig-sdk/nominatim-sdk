...
class FtCtrl extends NominatimControl
{
...
class FtClient
{
    ...
    private NominatimUtility $utility;

    public function __construct(NominatimUtility $utility, array $options)
    {
    ...
    public function get_utility(): NominatimUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NominatimContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NominatimContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NominatimContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NominatimConfig::make_feature($name);
    ...
    private static function buildUrl(NominatimSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NominatimError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NominatimResponse($fetched);
        }
    ...
    private function fail(NominatimContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NominatimContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
