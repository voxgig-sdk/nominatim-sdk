...
//     (\`new NominatimSDK(...)\` / \`NominatimSDK.test(...)\` become
//     \`NominatimSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"address_lookup":{"test01":{"id":"test01"}},"administrative":{"test01":{"id":"test01"}},"debug":{"test01":{"id":"test01"}},"reverse":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"server_status":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NominatimSDK(...)\` and any
// \`NominatimSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NominatimSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NominatimSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
