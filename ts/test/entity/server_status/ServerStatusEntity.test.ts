...
describe('ServerStatusEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'server_status.' + op, live)) return
    }
    ...
    let server_status_ref01_data = Object.values(setup.data.existing.server_status)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/server_status/ServerStatusTestData.json')
