...
module NominatimUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NominatimResponse.new({})
    ...
    if fetch_err
      ...
      response = NominatimResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NominatimResponse.new(fetched)
    else
