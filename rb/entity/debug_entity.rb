...
class DebugEntity
  ...
  # @return [Debug, Hash] the current Debug data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Debug fields)
  def match_get
  ...
  # Load a single Debug.
  #
  # @param reqmatch [DebugLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Debug.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Debug, Hash] the loaded Debug; raises NominatimError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NominatimHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
