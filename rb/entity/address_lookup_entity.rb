...
class AddressLookupEntity
  ...
  def make
    opts = @_entopts.dup
    AddressLookupEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NominatimHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [AddressLookup, Hash] the current AddressLookup data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NominatimHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of AddressLookup fields)
  def match_get
  ...
  #
  # @param reqmatch [AddressLookupListMatch, Hash, nil] match filter (any subset of
  #   AddressLookup fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<AddressLookup>, Array] the matching AddressLookup items; raises NominatimError on failure
