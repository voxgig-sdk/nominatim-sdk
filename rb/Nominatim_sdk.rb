...
require_relative 'Nominatim_types'
...
class NominatimSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NominatimHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NominatimHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NominatimFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NominatimUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NominatimHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NominatimHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NominatimHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = NominatimHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NominatimError.new(
        "graphql_error", "NominatimSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
