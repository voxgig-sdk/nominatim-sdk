...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = NominatimOmni.make_runner(TEST_JSON_FILE, NominatimSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : NominatimOmni.errify(err)
    end
