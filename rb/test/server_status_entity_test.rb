...
class ServerStatusEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = server_status_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "server_status." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NOMINATIM_TEST_SERVER_STATUS_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    server_status_ref01_data = nil
    if server_status_ref01_data_raw.length > 0
...
def server_status_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "server_status", "ServerStatusTestData.json")
  ...
  entid_env_raw = ENV["NOMINATIM_TEST_SERVER_STATUS_ENTID"]
