...
class AdministrativeDirectTest < Minitest::Test
  def test_direct_list_administrative
    setup = administrative_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-administrative", setup[:live] ? "live" : "unit")
    if _should_skip
...
def administrative_direct_setup(mockres)
