...
class AddressLookupEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = NominatimSDK.test(seed, nil)
    seen = base.AddressLookup(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = NominatimConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = NominatimSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.AddressLookup(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = address_lookup_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "address_lookup." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NOMINATIM_TEST_ADDRESS_LOOKUP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    address_lookup_ref01_data = nil
    if address_lookup_ref01_data_raw.length > 0
...
def address_lookup_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "address_lookup", "AddressLookupTestData.json")
  ...
  entid_env_raw = ENV["NOMINATIM_TEST_ADDRESS_LOOKUP_ENTID"]
