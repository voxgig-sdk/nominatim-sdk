...
class AddressLookupDirectTest < Minitest::Test
  def test_direct_list_address_lookup
    setup = address_lookup_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-address_lookup", setup[:live] ? "live" : "unit")
    if _should_skip
...
def address_lookup_direct_setup(mockres)
