...
type Args struct {
	Entity string         `json:"entity" jsonschema:"address_lookup | administrative | debug | reverse | search | server_status"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.NominatimSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "nominatim_list",
		Description: "List records from Nominatim. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "nominatim_load",
		Description: "Load a single record from Nominatim. " +
...
func runOp(client *sdk.NominatimSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.NominatimSDK, name string) (sdk.NominatimEntity, error) {
	switch strings.ToLower(name) {
	case "address_lookup":
		return client.AddressLookup(nil), nil
	case "administrative":
		return client.Administrative(nil), nil
	case "debug":
		return client.Debug(nil), nil
	case "reverse":
		return client.Reverse(nil), nil
	case "search":
		return client.Search(nil), nil
	case "server_status":
		return client.ServerStatus(nil), nil
