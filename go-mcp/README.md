...
[MCP](https://modelcontextprotocol.io) server exposing the Nominatim SDK as
two agent tools — `nominatim_list` and `nominatim_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nominatim-mcp)
make build

# 2. Provide credentials via the environment
export NOMINATIM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nominatim \
  -- /absolute/path/to/nominatim-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nominatim-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nominatim_list: first page of records
{ "entity": "address_lookup" }
{ "entity": "address_lookup", "query": { } }

// nominatim_load: one record by id
{ "entity": "debug", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nominatim-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nominatim \
     -- "$PWD"/dist/*/nominatim-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nominatim_list` and `nominatim_load` tools now appear
   in new sessions. Ask the agent to *"list address_lookup using nominatim"*
   and it calls `nominatim_list` with `{"entity":"address_lookup"}`.
...
```sh
export NOMINATIM_APIKEY=sk_live_xxx            # API key
export NOMINATIM_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `nominatim_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nominatim_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NOMINATIM_APIKEY` | API key sent with every request. |
| `NOMINATIM_BASE` | Optional override of the API base URL. |
...
```sh
./nominatim-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nominatim_load","arguments":{"entity":"debug","query":{"id":1}}}}'
```
