...
The Python SDK for the Nominatim API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.AddressLookup(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Nominatim(None).load(
    {"id": "test01"}, None
)
