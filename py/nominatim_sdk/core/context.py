...
from nominatim_sdk.core.helpers import get_ctx_prop, to_map
...
class NominatimContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, NominatimSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, NominatimResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, NominatimResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return NominatimError(code, msg, self)
