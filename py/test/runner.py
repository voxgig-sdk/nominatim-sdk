# Nominatim SDK test runner

from __future__ import annotations
import os
import json

from nominatim_sdk.utility.voxgig_struct import voxgig_struct as vs


class NominatimTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NominatimTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NominatimTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NominatimTestRunner.getenv("NOMINATIM_TEST_LIVE")
        override = NominatimTestRunner.getenv("NOMINATIM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NominatimTestRunner.getenv(key)
        ...
        explain = NominatimTestRunner.getenv("NOMINATIM_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NOMINATIM_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NominatimTestRunner._test_control is not None:
            return NominatimTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NominatimTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NominatimTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NominatimTestRunner.load_test_control()
...
def load_env_local():
    NominatimTestRunner.load_env_local()


def env_override(m):
    return NominatimTestRunner.env_override(m)


def entity_data(v):
    return NominatimTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NominatimTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NominatimTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NominatimTestRunner.load_test_control()


def live_client_options():
    return NominatimTestRunner.live_client_options()


def live_delay_ms():
    return NominatimTestRunner.live_delay_ms()
