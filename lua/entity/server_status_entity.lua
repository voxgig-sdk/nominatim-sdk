...
local ServerStatusEntity = {}
ServerStatusEntity.__index = ServerStatusEntity


function ServerStatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ServerStatusEntity)
  self._name = "server_status"
  self._client = client
...
function ServerStatusEntity:get_name()
  return self._name
end


function ServerStatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ServerStatusEntity.new(self._client, opts)
end
...
function ServerStatusEntity:mark_deleted()
  self._deleted = true
end


function ServerStatusEntity:deleted()
  return true == self._deleted
end


function ServerStatusEntity:data_set(args)
  if args ~= nil then
...
function ServerStatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ServerStatusEntity:match_set(args)
  if args ~= nil then
...
function ServerStatusEntity:match_get()
...
function ServerStatusEntity:stream(action, args, callopts)
...
---@param reqmatch ServerStatusLoadMatch
---@param ctrl? table
---@return ServerStatus
---@return string? err
function ServerStatusEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ServerStatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
