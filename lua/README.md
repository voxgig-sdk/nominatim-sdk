...
The Lua SDK for the Nominatim API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:AddressLookup(nil):list(nil, nil)
...
local result, err = client:Nominatim(nil):load(
  { id = "test01" }, nil
)
